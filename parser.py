...
class STNode(object):
    def __init__(self, ntype, value, line: int, *parts):
        self.type = ntype
        self.value = value
        self.lineno = line
        if parts:
...
class Parser:
    ...
    def p_stmt_list(self, p):
        """stmt_list : statement
                     | stmt_list statement"""
        if len(p) == 3:
            p[0] = STNode('conjunction', None, p.lexer.lineno, p[1], p[2])
        else:
            p[0] = p[1]
    ...
    def p_statement_while(self, p):
        ...
        p[0] = STNode('while', None, p.lexer.lineno, p[3], p[6])

    def p_statement_if(self, p):
        """statement : IF OPENBR expr CLOSEBR statement %prec IFX
                     | IF OPENBR expr CLOSEBR statement ELSE statement"""
        if len(p) == 6:
            p[0] = STNode('if', None, p.lexer.lineno, p[3], p[5])
        else:
            p[0] = STNode('if', None, p.lexer.lineno, p[3], p[5], p[7])
    ...
    def p_expr_unary(self, p):
        """expr : MINUS expr %prec UOPER
                ...
                | INC id %prec UOPER
                | DEC id %prec UOPER"""
        p[0] = STNode('unary', str(p[1]), p.lexer.lineno, p[2])
    ...
    def p_expr(self, p):
        """expr : expr PLUS expr
                ...
                | expr LT expr
                | expr EQ expr"""
        p[0] = STNode('oper', str(p[2]), p.lexer.lineno, p[1], p[3])
    ...
    def p_expr_ind1(self, p):
        ...
        p[0] = STNode('index_1d', None, p.lexer.lineno, p[1], p[3])

    def p_expr_ind2(self, p):
        """expr : id OPENBR expr CLOSEBR OPENBR expr CLOSEBR"""
        p[0] = STNode('index_2d', None, p.lexer.lineno, p[1], p[3], p[6])

    def p_expr_sz(self, p):
        """expr : SZ1 id %prec UOPER"""
        p[0] = STNode('size_1d', None, p.lexer.lineno, p[2])

    def p_expr_sz2(self, p):
        """expr : SZ2 id expr %prec UOPER"""
        p[0] = STNode('size_2d', None, p.lexer.lineno, p[2], p[3])
    ...
    def p_enum2(self, p):
        """enum2 : enum
                 | enum DCOMMA enum2"""
        if len(p) == 2:
            p[0] = [STNode('enum', p[1], p.lexer.lineno)]
        else:
            p[3].append(STNode('enum', p[1], p.lexer.lineno))
            p[0] = p[3]

    def p_declaration(self, p):
        """declaration : OPENIND enum_rets CLOSEIND FUNC id OPENBR opt_enum_ids CLOSEBR OPENST stmt_list CLOSEST"""
        p[0] = STNode('declare', 'function', p.lexer.lineno, p[5], p[10])
        retp = STNode('retp', p[2], p.lexer.lineno)
        pars = STNode('pars', p[7], p.lexer.lineno)
        p[0].add_part(retp)
        p[0].add_part(pars)
    ...
    def p_call(self, p):
        ...
        p[0] = STNode('call', p[5], p.lexer.lineno)
        sets = STNode('set', p[2], p.lexer.lineno)
        pars = STNode('pars', p[7], p.lexer.lineno)
        p[0].add_part(sets)
        p[0].add_part(pars)
    ...
    def p_enum_id(self, p):  # this will only match lists with at least 1 id inside!
        ...
        if len(p) == 2:
            ...
            p[6].append(STNode('index_1d', None, p.lexer.lineno, p[1], p[3]))
            p[0] = p[6]
        else:
            p[2].append(STNode('empty_slot', None, p.lexer.lineno))
            p[0] = p[2]
    ...
    def p_enum_id_2darr(self, p):
        ...
        if len(p) == 8:
            ...
            p[9].append(STNode('index_2d', None, p.lexer.lineno, p[1], p[3], p[6]))
            p[0] = p[9]
    ...
    def p_enum_par(self, p):
        """enum_par :
                    ...
                    | expr COMMA enum_par"""
        if len(p) == 1:
            p[0] = [STNode('empty_slot', None, p.lexer.lineno)]
        elif len(p) == 2:
            p[0] = [p[1]]
        elif len(p) == 4:
            p[3].append(p[1])
    ...
    def p_create_id(self, p):
        """create_id : UINT id ASGN expr
                     ...
                     | BOOL id ASGN expr
                     | CBOOL id ASGN expr"""
        p[0] = STNode('create_id', str(p[1]), p.lexer.lineno, p[2], p[4])
    ...
    def p_assign(self, p):
        ...
        if len(p) == 4:
            p[0] = STNode('assign', None, p.lexer.lineno, p[1], p[3])
        elif len(p) == 7:
            p[0] = STNode('assign', None, p.lexer.lineno, p[1], p[3], p[6])
        else:
            p[0] = STNode('assign', None, p.lexer.lineno, p[1], p[3], p[5], p[8])

    def p_id(self, p):
        """id : ID"""
        p[0] = STNode('id', p[1], p.lexer.lineno)
